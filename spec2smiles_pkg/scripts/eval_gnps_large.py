#!/usr/bin/env python
"""Evaluate GNPS Large model only - for parallel execution."""
...
def main():
    ...
    print(f"[LARGE] Using device: {device}")

    # Paths
    script_dir = Path(__file__).parent.parent
    foundry_dir = script_dir.parent
    gnps_path = foundry_dir / "smiles2spec/data/input/GNPS/spectral_data.jsonl"
    model_path = Path("/tmp/gnps_model/best_model.pt")

    if not model_path.exists():
        print(f"[LARGE] Model not found: {model_path}")
        return

    # Load data
    print("[LARGE] Loading GNPS data...")
    data = load_gnps_data(gnps_path)
    print(f"[LARGE] Total molecules: {len(data)}")

    # Split
    train_data, temp_data = train_test_split(data, test_size=0.2, random_state=42)
    val_data, test_data = train_test_split(temp_data, test_size=0.5, random_state=42)
    print(f"[LARGE] Test set: {len(test_data)} samples")

    # Build encoder
    print("[LARGE] Building encoder...")
    ...
    print(f"[LARGE] Vocab size: {encoder.vocab_size}")
    ...
    # Evaluate with smaller batches for progress updates
    batch_size = 100
    ...
    print(f"[LARGE] Evaluating with beam=5, n_return=10...")
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="[LARGE] Eval"):
    ...
    with open("/tmp/gnps_eval_large.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"[LARGE] Results saved to /tmp/gnps_eval_large.json")
