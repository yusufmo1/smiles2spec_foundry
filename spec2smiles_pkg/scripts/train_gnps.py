#!/usr/bin/env python
"""Train DirectDecoder on GNPS dataset (23K molecules)."""
...
from spec2smiles.data.augment import augment_dataset
...
def calculate_descriptors(smiles: str) -> list:
    """Calculate 30 molecular descriptors."""
...
def load_gnps_data(data_path: Path, max_samples: int = None):
    """Load and preprocess GNPS dataset."""
    ...
    for record in tqdm(data, desc="Descriptors"):
        ...
        # Convert explicit H SMILES to implicit if needed
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            continue
...
def train_model(
    ...
    # Update data with scaled descriptors
    ...
    if n_augment > 0:
        print(f"Augmenting training data ({n_augment}x)...")
        aug_smiles, aug_descriptors = augment_dataset(
            train_smiles, X_train_scaled.tolist(), n_augment=n_augment
        )
        train_smiles = aug_smiles
        X_train_scaled = np.array(aug_descriptors)
        print(f"  Original: {len(train_data)}, Augmented: {len(train_smiles)}")

    # Build vocabulary
    print("Building vocabulary...")
    encoder = SELFIESEncoder(max_len=100)

    train_selfies, train_valid = encoder.build_vocab_from_smiles(
        train_smiles, verbose=True
    )
    ...
    train_dataset = MolecularDataset(
        X_train_valid, train_smiles_valid,
        encoder=encoder, max_len=100
    )
    val_dataset = MolecularDataset(
        X_val_valid, val_smiles_valid,
        encoder=encoder, max_len=100
    )
    test_dataset = MolecularDataset(
        X_test_valid, test_smiles_valid,
        encoder=encoder, max_len=100
    )
    ...
    criterion = nn.CrossEntropyLoss(
        ignore_index=SELFIESEncoder.PAD_IDX, label_smoothing=0.1
    )
    ...
    for epoch in range(n_epochs):
        ...
        for batch in train_loader:
            ...
            loss = criterion(
                logits.view(-1, encoder.vocab_size),
                targets.reshape(-1)
            )
            loss.backward()
    ...
    for batch in tqdm(test_loader, desc="Evaluating"):
        ...
        candidates = model.generate_beam(
            descriptors, beam_width=40, n_return=100,
            length_penalty=0.6, diversity_penalty=0.0
        )
...
def main():
    # Paths - use relative to script location
    script_dir = Path(__file__).parent.parent  # spec2smiles_pkg
    foundry_dir = script_dir.parent  # smiles2spec_foundry

    gnps_path = foundry_dir / "smiles2spec/data/input/GNPS/spectral_data.jsonl"
    output_dir = Path("/tmp/gnps_model")
    ...
    # Train without augmentation first
    print("\n" + "="*70)
    print("TRAINING ON GNPS (NO AUGMENTATION)")
    print("="*70)
